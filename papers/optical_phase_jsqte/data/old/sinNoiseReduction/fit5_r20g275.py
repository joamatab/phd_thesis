...
#    p0 = [7*pi/4,0.5,30,4.36,-0.5,10e-3,2-3.45+1.25] # Initial guess for the parameters
    ...
    subplot(212)
    grid()
    plot(xpOverCoupled,ypOverCoupled/pi,xpOverCoupled,fitfunc2(p2,xpOverCoupled)/pi,'--')
#    ylim([-13,1])
    yticks([.3,.1,-.1,-.3],('0.3$\pi$','0.1$\pi$','-0.1$\pi$','-0.3$\pi$'))
    ylabel('Phase (rad)')
...
#    yfitdB = power2dBnormalized(yfit)
    subplot(211)
    grid()
    plot(xaOverCoupled,(yaOverCoupled),'-',xaOverCoupled,(fitfunc(p2,xaOverCoupled)),'--')
